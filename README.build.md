# tinyxml2: reproducible platform builds
...
Validated musl tag: [`musl-v11.0.0-1`](https://github.com/insightos-community/tinyxml2/releases/tag/musl-v11.0.0-1);
source commit: `72f639a28c70c056bde46c7ad7d80631a70dfd99`. Use a normal clone so container packaging can read `.git`.

```bash
git clone https://github.com/insightos-community/tinyxml2.git tinyxml2-repro
cd tinyxml2-repro
git checkout --detach musl-v11.0.0-1
test "$(git rev-parse HEAD)" = 72f639a28c70c056bde46c7ad7d80631a70dfd99
```
...
REPRO_WORK="$(mktemp -d "${TMPDIR:-/tmp}/tinyxml2-musl.XXXXXXXX")"
...
- `tinyxml2-11.0.0-musl-x86_64-prefix.tar.gz`
...
The macOS Pinocchio stack pins `cmeel-tinyxml2==10.0.0`, while this musl fork releases TinyXML2 11.0.0. A native build of this checkout is not a replacement for the locked macOS wheel.
...
```bash
gh workflow run musl-release.yml --repo insightos-community/tinyxml2 --ref musl-v11.0.0-1
gh run list --repo insightos-community/tinyxml2 --workflow musl-release.yml --limit 5
# Set REPRO_RUN_ID to the run ID printed above.
gh run watch "$REPRO_RUN_ID" --repo insightos-community/tinyxml2 --exit-status
gh run download "$REPRO_RUN_ID" --repo insightos-community/tinyxml2 --name musl-dist --dir downloaded-dist
```
